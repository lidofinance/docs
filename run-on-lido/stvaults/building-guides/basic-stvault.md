...
4. **Node Operator Fee**. The share of gross staking rewards that the Node Operator charges for providing validation services. Measured in basis points [0 (0%) .. 10'000 (100%)].
5. **Confirmation Lifetime**. The key parameter of the multi-role confirmation mechanism. It defines the maximum time interval between proposal and confirmation. This mechanism is used to update certain stVault parameters by requiring consensus between the two stVault representatives: the Vault Owner and the Node Operator Manager. For security reasons, it is strongly recommended to keep this interval as short as possible, ideally the minimal 86'400 sec. Measured in seconds [86'400 sec (24 hours) .. 25'920'000 sec (30 days)].
