...
![Multi-User Staking with Additional Yield via Custom DeFi Strategy](/img/stvaults/builders/architecture_public_custom.jpg)
...
Continue with [Post-deployment steps](#create-web-ui).

---

### Path B: Upgrade an existing pool to a strategy pool

Use this path when you have a running [`StvStETHPool`](./multi-user-delegated-staking.md/#deployment-of-stvstethpool-pool-with-steth-minting) and want to add a strategy without redeploying the pool. All existing user balances and state are preserved through the proxy upgrade.
...
:::info
Steps 6–7 (revoke pause roles from the Node Operator) adjust the emergency role setup to match the strategy pool configuration. Review the [DeFi Wrapper roles and permissions](../../vault-owners-curators-and-stakers/defi-wrapper/vault-owners-and-curators/roles-and-permissions.md) to decide what role assignment is appropriate for your setup.
:::
...
For more information about how this process works for the Basic stVault, please follow [Adjust stETH minting parameters](../../node-operators/basic-stvaults/stvault-tier-and-steth-minting-limit.md).
