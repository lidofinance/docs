...
The PredepositGuarantee (PDG) contract mitigates deposit frontrunning vulnerabilities outlined in [LIP-5](https://github.com/lidofinance/lido-improvement-proposals/blob/develop/LIPS/lip-5.md). It disincentivizes frontrunning by having the Node Operator post an economic guarantee of honest behavior, which is proven/disproven via [EIP-4788](https://eips.ethereum.org/EIPS/eip-4788). This mechanism is distinct from the [Deposit Security Module](/contracts/deposit-security-module) used by Lido Core.

The PDG enables Node Operators to deposit validators using stVault funds in a trustless manner.
...
1. The Vault Owner supplies 2048 ETH (minimum 32 ETH required to activate a validator) to the stVault.
...
6. The `Depositor` predeposits 1 ETH from the stVault balance to the validator via the PDG contract.
...
9. The `Depositor` makes a top-up deposit of the remaining 2016 ETH from the stVault balance to the validator through the PDG.
...
The shortcut skips the predeposit step: up to 2048 ETH (as per [EIP-7251](https://eips.ethereum.org/EIPS/eip-7251)) goes to the validator in a single transaction, without PDG. The validator can be linked to the stVault after the deposit by submitting proof through the PDG contract.
...
1. The Vault Owner supplies ETH to the stVault but does not mint against it.
...
2. The Vault Owner gives the Node Operator an explicit permission to bypass PDG by setting the appropriate PDG policy.
   ...
   The Vault Owner opens the **Main settings** page of the stVault and picks the policy under **Predeposit Guarantee Policy**.
...
3. **(Optional)** The Node Operator Manager grants the necessary roles to the address that will perform unguaranteed deposits and proving. By default, the Node Operator Manager can perform unguaranteed deposits. Both roles are assumed to go to the same address, referred to as `Depositor` below.
...
6. The `Depositor` performs a deposit in the specified amount from the stVault balance to the validator via the Dashboard contract.
...
9. **(Optional)** The `Depositor` makes a top-up deposit from the stVault balance to the validator through PDG.
...
- [stVaults Technical Design](../../concepts-and-reference/stvaults-technical-design.md) — how PDG fits the rest of the protocol
