...
Ethereum is and remains a primary focus of Lido. That’s affected the organisational structure - the governance is implemented through ERC20 `LDO` token. Currently Lido supports also the Polygon PoS network - when staking native tokens with Lido, users will receive a `st[token]`, which represents their `[token]` which represents their deposit and the rewards aggregated over time (penalties too).

<Grid>

<GridLink
  to="https://docs.polygon.lido.fi"
  title="Lido on Polygon (stMATIC)"
  subtitle="For the Lido on Polygon protocol documentation please proceed to a separate portal."
  Icon={<PolygonIcon />}
/>
