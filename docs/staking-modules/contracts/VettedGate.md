...
[`MerkleGateFactory`](/staking-modules/contracts/MerkleGateFactory) deploys gate instances behind [OssifiableProxy](/contracts/ossifiable-proxy). Each instance has its own participant list, bond curve, and administrator, so its eligibility list can be updated independently.
