...
| Module | Mainnet |
| --- | --- |
| CMv2 | [`0xA64b339eebD3dC3De848298B6a140955932901d8`](https://etherscan.io/address/0xA64b339eebD3dC3De848298B6a140955932901d8) |

`MetaRegistry` is a supplementary contract that stores Node Operator metadata and defines operator groups used by [`CuratedModule`](/staking-modules/contracts/CuratedModule) for weighted stake allocation. A group assigns shares to its module Node Operators and can reference operators in external staking modules. The registry calculates each grouped Node Operator's effective allocation weight from its [`Accounting`](/staking-modules/contracts/Accounting) bond curve and its share within the group. Operators outside a group have zero allocation weight.
