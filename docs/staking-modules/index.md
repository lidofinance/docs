...
Node Operators get their rewards from the bond rebase and from their [portion of the staking rewards](/staking-modules/rewards). Accumulated CL penalties resulting in a withdrawal balance below the validator's [confirmed expected balance](/staking-modules/validator-exits#validator-balance-tracking), as well as stolen EL rewards, are deducted from the bond. Node Operators should perform validator exits upon protocol request to avoid force ejection (via [EIP-7002](https://eips.ethereum.org/EIPS/eip-7002)), and can also voluntarily [exit or eject](/staking-modules/validator-exits) their validators.
...
- **Rewards.** Node Operator rewards are allocated by a Performance Oracle over per-module frames (14 days in CMv2 and 28 days in CSM on Mainnet) and published in a cumulative Merkle tree, on top of the bond rebase. See [Rewards](/staking-modules/rewards).
