...
### Top-up queue
The top-up queue applies to the [`0x02` module](#0x02-validators-support), where validators are funded in two phases: an initial `32 ETH` deposit allocated through the [stake distribution queue](#stake-distribution-queue) above, followed by top-ups up to `2048 ETH`. Top-ups follow a dedicated top-up queue whose order matches the order in which the initial `32 ETH` deposits were made (this order is recorded at the time of the initial deposit).

### 0x02 validators support
[CSM v3](https://github.com/lidofinance/staking-modules/releases/tag/v3.0) introduces native support for `0x02` validator withdrawal credentials. Each CSM deployment serves a single withdrawal-credential type, so the existing CSM continues to serve `0x01` validators only, while `0x02` validators are served by a separate module that conforms to the [`IStakingModuleV2`](https://github.com/lidofinance/core/blob/af095e48bbc1c3841c2c9936219c8461af01056b/contracts/common/interfaces/IStakingModuleV2.sol) interface. In the `0x02` module, each validator first receives an initial `32 ETH` deposit and is later topped up to `2048 ETH`. More info about the `0x02` CSM can be found in [the corresponding proposal](https://research.lido.fi/t/0x02-csm-landscape/11697).
