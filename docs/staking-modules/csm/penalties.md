...
3. **The operator has not exited the validators in time.** Charge amount = `exitDelayFee`, a fixed amount set by the DAO that is [scaled with the validator's balance](#penalty-scaling-for-larger-validators);
...
Most reasons in the previous section result in a penalty (burn). The amounts applied as charges are the `keyRemovalCharge`, the delayed-exit fee (`exitDelayFee`), and the EL withdrawal request fee, capped by `maxElWithdrawalRequestFee`.
