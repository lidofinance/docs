...
2. **The validator's withdrawal balance is less than its deposited balance** (`32 ETH` for `0x01` validators, up to `2048 ETH` for `0x02` validators). Covers cases of a meaningful downtime and slashing. Penalty amount = `deposited balance - validator's withdrawal balance`. This reflects the actual loss, so it is not scaled. For slashed validators, an explicit slashing penalty can be reported instead (see [Validator Exits](./validator-exits.md#withdrawal-balance-reporting));
...
The second penalty is calculated from the validator's withdrawal balance and applied immediately in the reporting transaction. See [Withdrawal balance reporting](./validator-exits.md#withdrawal-balance-reporting) for the reporting flows for slashed and non-slashed validators.
