...
- [Source code](https://github.com/lidofinance/community-staking-module/blob/v2.0/src/CSModule.sol)
...
_This method is not used in CSM, hence it does nothing_

_NOTE: No role checks because of empty body to save bytecode._
...
_This function is called by the StakingRouter to report the current exit-related status of a validator
...
This data could be used to trigger penalties for the node operator if the validator has exceeded the allowed exit window._
...
_This function is called by the StakingRouter when a validator is exited using the triggerable
exit request on the Execution Layer (EL)._
...
_The method does not update depositable keys count for the Node Operators before the queue processing start.
...
_This view must not revert in case of invalid data passed. When `offset` exceeds the
total node operators count or when `limit` is equal to 0 MUST be returned empty array._
...
_This function is used to get the accounting contract from immutables to save bytecode and for backwards compatibility_
