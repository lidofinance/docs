...
This document is intended for developers looking to integrate stETH as an asset into their DeFi protocol, with a focus on money markets, DEXes and blockchain bridges.
...
The stETH token balances are being recalculated daily when the Lido oracle reports Beacon chain ether balance update. The balance update happens automatically on all the addresses holding stETH at the moment of rebase. This mechanics has been implemented via **shares**.
...
When integrating stETH as an assets into any dApp, it's highly recommended to store and operate shares rather than stETH public balances directly, because stETH balances change both upon transfers, mints'burns, and rebases, while shares balances can only change upon transfers and mints/burns.
