...
- Download and install foundry cast to check every stated fact about contract's abi, address, roles assigned, proxy admins, etc.
- If something is not clear better create a separate question and put in into the `TO_RESOLVE.md`
- You can also utilize treat this release https://github.com/lidofinance/core/releases/tag/v3.0.1 to be the latest and deployed on-chain
- https://github.com/lidofinance/core/blob/v3.0.1/deployed-mainnet.json treat this as a source of truth for the latest mainnet deployments

## Approach
1. Map each review comment to the exact doc section and required change. You can find that some of them are already implemented.
...
4. Normalize internal links and address formatting across all touched docs, use Etherscan links for every address, put in ticks (``) for better readability
...
6. Do a check using foundry cast to check every stated fact about contract's abi, address, roles assigned, proxy admins, etc.
7. Treat sources of truth as ultimate MUST cross-check sources.
